...
namespace Celeste.Mod.ChineseNewYear2023Helper.POMR.Entities.Placables {
    [Tracked]
    [CustomEntity("ChineseNewYear2023Helper/PlacableRenewableDoubleDashRefill")]
    public class PlacableRenewableDoubleDashRefill : PlacableEntity {
        public PlacableRenewableDoubleDashRefill() : base() {
            base.Type = Placable.REFILL_RENEWABLE_DOUBLEDASH;
        ...
        public override void Recycle() {
            POMRController.List.ForEach(item => {
                if (item is RenewableDoubleDashRefillItem) {
                    item.Count += 1;
        ...
        public override void Spawn() {
            base.SceneAs<Level>().Add(new Refill(Position, true, false));
        }
    ...
    public class RenewableDoubleDashRefillItem : PlacableItem {
        public RenewableDoubleDashRefillItem(int cost, int count) : base(cost, count) {
            base.Type = Placable.REFILL_RENEWABLE_DOUBLEDASH;
            base.Texture = GFX.Gui["ChineseNewYear2023/optimize_2/items/renewabledoubledashrefill"];
        }

        public override bool Place(Vector2 pos) {
            PlacableEntity entity = new PlacableRenewableDoubleDashRefill();
